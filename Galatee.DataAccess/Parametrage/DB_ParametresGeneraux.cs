...
namespace Galatee.DataAccess
{
    public class DB_ParametresGeneraux /*: Galatee.DataAccess.Parametrage.DbBase*/
    {
        ...
        */
        ...
        public List<CsCategorieClient> ChargerListeExemtion()
        {
            ...
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.CommandText = "SPX_PARAM_SELECT_CATEGORIE_EXEMPTION";
        ...
        public string TraiterExemptionCategorie(List<CsCategorieClient> lesCategories)
        {
            SqlCommand laCommande = DBBase.InitTransaction(Session.GetSqlConnexionString());
            try
            {
                Delete_Exemption(laCommande);

                if (lesCategories != null && lesCategories.Count != 0)
                {
                    foreach (CsCategorieClient item in lesCategories)
                        InsertExemptionCategorie(item, laCommande);
                }
        ...
        private void Delete_Exemption(SqlCommand cmds)
        {
            cmds.CommandTimeout = 180;
            cmds.CommandType = CommandType.StoredProcedure;
            if (cmds.Parameters != null && cmds.Parameters.Count != 0) cmds.Parameters.Clear();

            cmds.CommandText = "SPX_PARAM_DELETE_EXEMPTION_CATEGORIE_INTERET_MORATOIRE";
        ...
        public void InsertExemptionCategorie(CsCategorieClient laCategorie, SqlCommand cmds)
        {
            cmds.CommandTimeout = 1800;
            cmds.CommandType = CommandType.StoredProcedure;
            cmds.CommandText = "SPX_PARAM_INSERTEXEMPTION_INTERET_MORATOIRE";
            if (cmds.Parameters != null && cmds.Parameters.Count != 0) cmds.Parameters.Clear();

            cmds.Parameters.Add("@USERCREATION", SqlDbType.VarChar, 6).Value = laCategorie.USERCREATION;
            cmds.Parameters.Add("@FK_IDCATEGORIE", SqlDbType.Int).Value = laCategorie.PK_ID;
