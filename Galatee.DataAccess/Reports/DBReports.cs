...
namespace Galatee.DataAccess
{
    public class DBReports
    {
        ...
        public List<CsLclient> RetourneInteretMensuel(List<int> lstCentre, List<int> lstCategorie, List<int> lstRegroupement, string PeriodeDebut, string periodefin, string etat)
        {
            try
            {
                string Centre = DBBase.RetourneStringListeObject(lstCentre);
                string Categorie = DBBase.RetourneStringListeObject(lstCategorie);
                string Regroupement = DBBase.RetourneStringListeObject(lstRegroupement);
                ...
                if (etat == Enumere.ListeInteretsMoratoireDus)
                    cmd.CommandText = "SPX_RPT_LISTE_IMO_DUS";
                if (etat == Enumere.ListeInteretsMoratoirePayés)
                    cmd.CommandText = "SPX_RPT_LISTE_IMO_PAYES";
                if (etat == Enumere.ListeInteretsMoratoireAnnulés)
                    cmd.CommandText = "SPX_RPT_LISTE_IMO_ANNULES";

                cmd.Parameters.Add("@LesCentres", SqlDbType.VarChar, int.MaxValue).Value = Centre;
                cmd.Parameters.Add("@LesCategories", SqlDbType.VarChar, int.MaxValue).Value = Categorie;
                cmd.Parameters.Add("@LesRegroupements", SqlDbType.VarChar, int.MaxValue).Value = Regroupement;
                cmd.Parameters.Add("@PeriodeDebut", SqlDbType.VarChar, 6).Value = string.IsNullOrEmpty(PeriodeDebut) ? null : PeriodeDebut;
                cmd.Parameters.Add("@PeriodeFin", SqlDbType.VarChar, 6).Value = string.IsNullOrEmpty(periodefin) ? null : periodefin;
                DBBase.SetDBNullParametre(cmd.Parameters);
                try
                {
                    if (cn.State == ConnectionState.Closed)
