...
namespace Galatee.DataAccess
{
    public class DbInterfaceComptable
    {
            ...
            try
            {
                ...
                foreach (CsOperationComptable Operation in lesOperationCpt.Where(p=>p.CODE != Enumere.FactureTravaux).ToList())
                {
                    foreach (int item in IdCentre)
                    {
                        ...
                        else if (Operation.CODE == Enumere.FactureInteretMoratoire)
                            lstEcriture.AddRange(RetourneComptabilisationIMOSpx(item, null, DateCaisseDebut, DateCaisseFin));
                    }
                }
            ...
            try
            {
                ...
                foreach (CsOperationComptable Operation in lesOperationCpt)
                {
                    foreach (CsCaisse item in lstCaisse)
                    {
                        ...
                        else if (Operation.CODE == Enumere.EncaissementInteretMoratoire)
                            lstEcriture.AddRange(RetourneComptabilisationIMOSpx(null, item.PK_ID, DateCaisseDebut, DateCaisseFin));
                    }
                }
        ...
        public List<CsComptabilisation> RetourneComptabilisationIMOSpx(int? IdCentre, int? IdCaisse, DateTime? DateCaisseDebut, DateTime? DateCaisseFin)
        {
            try
            {
                cn = new SqlConnection(ConnectionString);

                int? IdCentre_ = IdCentre != null ? IdCentre : null;
                int? IdCaisse_ = IdCaisse != null ? IdCaisse : null;

                cmd = new SqlCommand();
                cmd.Connection = cn;
                cmd.CommandTimeout = 3000;
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.CommandText = "SPX_CPT_INTERET_MORATOIRE";
                cmd.Parameters.Add("@Idcaisse", SqlDbType.Int).Value = IdCaisse_;
                cmd.Parameters.Add("@IdCentre", SqlDbType.Int).Value = IdCentre_;
                cmd.Parameters.Add("@DateCaisseDebut", SqlDbType.DateTime).Value = DateCaisseDebut;
                cmd.Parameters.Add("@DateCaisseFin", SqlDbType.DateTime).Value = DateCaisseFin;
                ...
                try
                {
                    ...
                    if (IdCaisse_ != null)
                        lstEntfact.ForEach(t => t.COPER = Enumere.CoperRGT );

                    lstEntfact.ForEach(t => t.COPERINITAL = Enumere.CoperIMO );
                    lstEntfact.ForEach(t => t.DC = Enumere.Debit);
