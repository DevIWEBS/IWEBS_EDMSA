        ...
        public CsDemande RetourneDetailClientFromRefClient(int fk_idcentre, string centre, string client, string ordre, string produit,string Typedemande)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(produit))
                    dtA = AccueilProcedures.RetourneAbonProduit(fk_idcentre, centre, client, ordre, produit);
                else
                    dtA = AccueilProcedures.RetourneAbon(fk_idcentre, centre, client, ordre);
...
*/
        ...
        public CsDemandeBase CreeDemande(CsDemande laDemande, bool AvecTransmission)
        {
            ...
            try
            {
                ...
                /* 05/05/2021 selectionner la facture a asc a rembourser */
                #region Coutdemande
                if (laDemande.LaDemande.TYPEDEMANDE == Enumere.RemboursementAvance  && laDemande.LstCoutDemande != null && laDemande.LstCoutDemande.Count != 0)
                {
...
*/
        ...
        */
        ...
        public bool ValiderIMO(int id, string matricule)
        {
            SqlCommand laCommande = DBBase.InitTransaction(Session.GetSqlConnexionString());
            try
            {
                InsertAnnulation(id, matricule, laCommande);
                laCommande.Transaction.Commit();
                return true;
        ...
        private void InsertAnnulation(int id, string matricule, SqlCommand cmds)
        {
            cmds.CommandTimeout = 1800;
            cmds.CommandType = CommandType.StoredProcedure;
            cmds.CommandText = "SPX_ACC_VALIDATION_ANNULATION_IMO";
            if (cmds.Parameters != null && cmds.Parameters.Count != 0) cmds.Parameters.Clear();

            cmds.Parameters.Add("@PK_ID", SqlDbType.Int).Value = id;
            cmds.Parameters.Add("@MATRICULE", SqlDbType.VarChar, 6).Value = matricule;
