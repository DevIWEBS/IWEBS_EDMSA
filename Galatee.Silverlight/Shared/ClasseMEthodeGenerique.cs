...
namespace Galatee.Silverlight.Shared
{
    static class ClasseMEthodeGenerique
    {
        ...
        /* LKO retourne distinct centre de la liste 10/05/2021 */
        public static List<CsClient> RetourneDistinctCentre(List<CsClient> _LstClient)
        {
            try
            {
                List<CsClient> _lstClientDistinct = new List<CsClient>();
                var ListClientTemp = (from p in _LstClient
                                     group new { p } by new { p.FK_IDCENTRE,p.LIBELLECENTRE   } into pResult
                                     select new
                                     {
                                         pResult.Key.FK_IDCENTRE,
                                         pResult.Key.LIBELLECENTRE 
                                     });
                foreach (var item in ListClientTemp)
                {
                    CsClient leCentre = new CsClient()
                    {
                        FK_IDCENTRE = item.FK_IDCENTRE ,
                        LIBELLECENTRE = item.LIBELLECENTRE
                    };
                    _lstClientDistinct.Add(leCentre);
                }
                return _lstClientDistinct;
            }
            catch (Exception ex)
            {
