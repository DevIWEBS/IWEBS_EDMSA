        ...
        static public List<CsReglementRecu> ReorganiserReglement(List<Galatee.Silverlight.ServiceCaisse.CsLclient> lstInit, string Action)
        {
            try
            {
                ...
                foreach (CsLclient item in reglemntParAcquit.OrderBy(t => t.MODEREG).ThenBy(u=>u.LIBELLECOPER).ThenBy(p => p.NDOC))
                {
                    ...
                    /* Correction de la date de reglement 18/04/2021 LKO*/
                    ObjImpression.DATEENCAISSEMENT  = System.DateTime.Now;
                    if ("Duplicata"== Action )
                        ObjImpression.DATEENCAISSEMENT = item.DATECREATION ;

                    if (item.COPER == SessionObject.Enumere.CoperEXO && SessionObject.LstDesCopers != null && SessionObject.LstDesCopers.Count != 0)
                        item.LIBELLECOPER = SessionObject.LstDesCopers.FirstOrDefault(y => y.CODE == SessionObject.Enumere.CoperEXO).LIBELLE;
